...
export default function EditQuotePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: its } = await supabase
        ...
        .eq("quote_id", id)
        .order("sort_order");
      if (its && its.length > 0) setItems(its);
  ...
  const save = async () => {
    ...
    // Replace line items: delete existing, then insert current set
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <h1 className="text-2xl font-bold">Edit Quote {quoteNumber}</h1>
      ...
      <div className="flex gap-2">
        ...
        <button className="btn-secondary" onClick={() => router.push(`/quotes/${id}`)}>
          Cancel
        </button>
      </div>
    </div>
